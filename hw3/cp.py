...
class shuf:
    def __init__(self, filename, repeat, dothis, numlines= "", low="", hi=""):
        if(filename == ""):   
            self.shufNums(repeat, dothis,  numlines, low, hi)
        elif(filename != "-"):
            f = open(filename, 'r')
            self.lines = f.readlines()
            f.close()
            self.choice(dothis, numlines, repeat)
        else:
            self.lines = sys.stdin.readlines()
            self.choice(dothis, numlines, repeat)
    def choice(self, dothis, numlines, repeat):
    ...
    def shufNums(self, repeat, dothis, numlines, low, hi):
        ...
        for i in range(0, len(arr)):
            ...
            num+=1
        if(repeat):
            self.repeatOk(numlines,dothis, arr)
        elif(numlines == 0):
            self.permute(arr)
        else:
    ...
    def repeatOk(self, numlines, dothis, arr):
        if dothis == True:
            while(True):
...
Permute lines from FILE. If no FILE is specified, read from standard input."""
   ...
   # low =""       
    ...
    if (options.inputr):
        ...
        try:
            shuf("",repeat,dothis,numlines,low, hi)
        except IOError as e:
       ...
       try:
           shuf(input_file, repeat ,dothis, numlines) 
       except IOError as e:
           parser.error("I/O error({0}): {1}".
                     format(e.errno, e.strerror))
