...
struct pixelColor image[width][height];
  ...
  Vec3 camera_pos;
    ...
    const double subsample_dy
        = halfSamples ? pixel_dy / ((double)halfSamples*2.0)
                      : pixel_dy;
    int limit=((*(int*)(pos))+1)*(width/nthreads);
    if((*(int*)(pos)) == nthreads-1)
       limit = width;
  for( int px=(*(int*)(pos))*(width/nthreads); px<limit; ++px )
  {
 ...
 *                           */
            ...
            image[px][py].r = scaled_color[0];
            image[px][py].g = scaled_color[1];
            image[px][py].b = scaled_color[2];
        }
    }
return NULL;
}
...
main( int argc, char **argv )
{
   ...
   /* for every pixel */
    ...
    int* pos = (int*) malloc(nthreads*sizeof(int));
    for(int i = 0; i < nthreads; i++)
    {
       pos[i] = i;
       int ret = pthread_create(&tids[i], NULL, func,(void*) &pos[i]);
       if(ret)
       {
